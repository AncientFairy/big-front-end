...
* main盒子宽度为980px，位置距离左边220px，给高度就不用清除清除浮动了。
* main里面包含左侧盒子，左浮动，focus焦点图模块。
* main里面包含右侧盒子，右浮动，newsflash新闻快报模块。

![main主体模块制作](images/main主体模块制作.png)
...
* 有高度可以不清除浮动。
* 左边的h3盒子左浮动。
* 右边tab-list，右浮动，因为用到tab切换效果，所以里面需要ul和li来做。

### 3.10.3 tab栏原理--布局需求

* 要求选项卡的个数要和内容个数一致。

![tab栏原理布局需求](images/tab栏原理布局需求.png)

### 3.10.4 box_bd模块

* 根据tab切换的原理，tab-content里面包含内容部分，可以通过ul布局。
* 分为5个大列，列的宽度不一致。

![楼层floor之box_bd 模块](images/楼层floor之box_bd 模块.png)
